...
def all_odd(numbers):
    ...
    # initializing an empty list to append odd numbers to
    odd_numbers = []

    for number in numbers:
        # if the remainder of the number modulo 2 is 1, that means it's
        # an odd number
        if number % 2 == 1:
...
def foods_in_common(foods1, foods2):
    ...
    **NOTE**: for this problem, you're welcome to use any of the
    ...
    # initializing an empty list to append common foods to
    common_foods_list = []

    for food in foods1:
        # if this food item from foods1 exists in the foods2, append it to the
        # common_foods_list
        if food in foods2:
            common_foods_list.append(food)
        elif len(foods1) == 0:
            return []
...
def every_other_item(items):
    ...
    # initializing an empty list to append every other item to
    every_other_item = []

    for item in items:
        if items.index(item) == 0:
            every_other_item.append(item)
        elif items.index(item) % 2 == 0:
            every_other_item.append(item)
    # sorry - i can't seem to get rid of this white space           
